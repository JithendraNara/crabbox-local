...
- **Daytona, Islo, E2B, and CF Containers.** Set `provider: daytona` for Daytona SDK/toolbox execution from a snapshot with explicit SSH access when needed, `provider: islo` for delegated Islo sandbox execution through the Islo Go SDK, `provider: e2b` for delegated E2B sandbox execution through E2B sandbox APIs, or `provider: cf-containers` for delegated Cloudflare Containers execution through a Worker runner.
