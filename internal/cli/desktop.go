...
if [ "$browser_wrapper" = "/usr/local/bin/crabbox-browser" ] && [ -f "$browser_wrapper" ] && {
  ...
  { [ -f /var/lib/crabbox/desktop.env ] && grep -Eq '^CRABBOX_DESKTOP_ENV=(wayland|gnome)$' /var/lib/crabbox/desktop.env && ! grep -q -- "--ozone-platform=wayland" "$browser_wrapper" 2>/dev/null; }
}; then
