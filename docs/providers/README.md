...
| [CF Containers](cf-containers.md) | delegated run | Linux | Cloudflare Containers execution through a Worker runner |
...
- CF Containers uses a deployed Worker runner backed by a Cloudflare
  Containers image.
...
| CF Containers | yes | yes | no | no | archive via Worker runner | no |
